...
function createFormEventListener(_state, onValidationSuccess, redraw, anchorElement) {
  const state = _state;
  return (e) => {
    e.preventDefault();
    const form = e.target;
    ...
    validate(valueToCheck, state.alreadyUsedRss).then((validationResult) => {
      ...
      form.reset();
      form.focus();
      redraw(state, anchorElement, onValidationSuccess);
    });
  };
}

function createForm(state, onValidationSuccess, redraw, anchorElement) {
  const form = document.createElement('form');
  form.setAttribute('action', '');
  form.classList.add('rss-form', 'text-body');
  form.addEventListener('submit', createFormEventListener(state, onValidationSuccess, redraw, anchorElement));
...
function draw(state, anchorElement, onValidationSuccess) {
  ...
  const form = createForm(state, onValidationSuccess, draw, anchorElement);
